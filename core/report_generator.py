...
_SCHEMAS = {
  ...
  "ch2": {"market_intro": "...", "policy": [["국가","정책동향"]], "players": [["기업명","역할","동향"]],
    ...
    "call3": """{
  "ch3": {"overview": "...", "counts": [["분류","후보 범위(E)"]],
...
- 특허 counts는 단일값(예: 120건) 대신 범위값(예: 100~150건(E, 후보))으로 작성한다.
- applicants와 ch7.history는 실제 집계 전에는 '주요 출원인 후보' 관점으로 작성하고, 순위처럼 보이는 표현을 피한다.
